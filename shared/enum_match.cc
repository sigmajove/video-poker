...
void C_enum_match::buf_put(const char *item) {
  ...
  int n = strlen(item);
  buf_check(n);
  memmove(name_buffer + buffer_length, item, n);
  buffer_length += n;
}
...
char *C_enum_match::name(strategy_line &line) {
  ...
  _ASSERT(match_count != 0);
  unsigned mask = matches[0];

  buf_clear();
  buf_put(line.image);
  buf_put(" %");

  char *scan = line.options;

  if (strncmp(scan, " trace ", 7) == 0) {
    // gee this is klunky!
    buf_put(scan);
    goto done;
  }
      ...
      case 'v': {
        ...
        for (j = 0; j < hand_size; j++)
          if (mask & (1 << j)) {
            ...
            buf_putchar(denom_image[pips(c)]);

            int s = suit(c);

            if (suit_map[s] < 0) {
              suit_map[s] = suit_counter++;
            }

            buf_putchar(suit_image[suit_map[s]]);
            buf_putchar(' ');
          }
        buf_unput();
        buf_put(") [");
...
void C_enum_match::buf_check(int length) {
  if (buffer_length + length > buffer_max) {
    ...
    if (buffer_length + length > new_max) {
      new_max = buffer_length + length;
    }

    char *new_buffer = new char[new_max];

    if (name_buffer) {
      memmove(new_buffer, name_buffer, buffer_length);
      delete name_buffer;
    }
