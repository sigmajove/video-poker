...
denom_list::denom_list(int *left_table) : left(left_table) {
...
int denom_list::no_pair(int n) {
  ...
  const int sum4 = avail[4];
  const int sum3 = avail[3] + sum4;
  const int sum2 = avail[2] + sum3;
  const int sum1 = avail[1] + sum2;
  ...
  const int max1 = std::min(avail[1], n);
  for (int j1 = min1; j1 <= max1; j1++) {
    int f1 = combin.choose(avail[1], j1);
    n -= j1;

    const int min2 = std::max(n - sum3, 0);
    const int max2 = std::min(avail[2], n);
    for (int j2 = min2; j2 <= max2; j2++) {
      const int f2 = combin.choose(avail[2], j2) * (1 << j2) * f1;
      // (1<<j2) == 2**j2

      n -= j2;

      const int min3 = std::max(n - sum4, 0);
      const int max3 = std::min(avail[3], n);
      for (int j3 = min3; j3 <= max3; j3++) {
        int power = 1;
        for (int jj = 0; jj < j3; jj++) power *= 3;
        // power == 3**j3

        const int f3 = combin.choose(avail[3], j3) * power * f2;

        const int j4 = n - j3;
        assert(0 <= j4 && j4 <= avail[4]);

        const int f4 = combin.choose(avail[4], j4) * (1 << (2 * j4)) * f3;
        // (1<<(2*j4)) == 4**j4
